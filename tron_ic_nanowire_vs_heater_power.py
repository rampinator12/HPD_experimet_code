...
#==============================================================================
device = 2.7
voltages = np.geomspace(0.01,2,201)
heater_width = 0.175e-6
rbias = 1e3
data_list = []
...
#%%
df = pd.read_csv(r"C:\Users\vacnt\Downloads\2021-12-01-15-34-42-IV-sweepic_heater_power (2).csv")

df['power_density'] = df['power']*1e9/(df['heater_width']*1e6)**2

plt.semilogx(df['power_density'], df['ic_median']*1e6, ".:")
plt.xlabel('power density (nW/um^2)')
plt.ylabel('ic (uA)')
plt.title('Device 3.1')
df.to_csv(filename + 'ic_heater_power.csv')
plt.savefig(filename+ 'ic_heater_power.csv.png', dpi = 300)
filename = datetime.datetime.now().strftime('%Y-%m-%d %H-%M-%S IV sweep')
